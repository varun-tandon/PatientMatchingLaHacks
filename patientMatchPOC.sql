...
USE DataExport DECLARE @OASALT VARCHAR(25)
SELECT
    @OASALT = 'OATEST' IF OBJECT_ID('tempdb..#tmpHASH') IS NOT NULL DROP TABLE #tmpHASH;
    WITH cteImport AS (
        SELECT
            DISTINCT TOP 10000 P.LAST_NAME LastName,
        ...
        SELECT
            ...
            (
                HASHBYTES(
                    ...
                    (
                        @OASALT + (
                            CONVERT([CHAR](8), [DateOfBirth], (112)) + '~' + [Gender]
                        ) + '~' + [FirstName]
                    ) + '~' + [LastName]
                )
            ) [FullNameHash],
        ...
        FROM
            (
                SELECT
                    ...
                    UPPER(
                        OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), FirstName))
                    ) [FirstName],
                    UPPER(
                        OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), LastName))
                    ) [LastName],
                    C.LastName orgLastName,
                    C.FirstName orgFirstName --, '' MiddleName
                FROM
                    cteImport C
            ) X
    ),
...
SELECT
    ...
    TH.ParHash,
    'AP' + RIGHT(
        '00000000' + CONVERT(VARCHAR(10), 100 + C.hashID),
        8
    ) xRefID,
    DATEADD(
        DAY,
    ...
    TH.Gender,
    TH.orgLastName,
    TH.orgFirstName INTO #tmpHash
FROM
    cteHASH TH
    JOIN cteHIDX C ON C.idx = TH.idx
ORDER BY
    TH.idx IF OBJECT_ID('tempdb..#tmpMatch') IS NOT NULL DROP TABLE #tmpMatch
...
SELECT
    TOP 100 PERCENT c.idx,
    c.hashID,
    c.ParHash,
    PHH.ClaimID INTO #tmpMatch
FROM
    cte c
    JOIN OADWH_HASH.dbo.vPatientHash_HCFA PHH WITH (NOLOCK) ON PHH.ParHash = c.ParHash 

IF OBJECT_ID('tempdb..#tmpStageName') IS NOT NULL DROP TABLE #tmpStageName
...
,
    ...
    CASE
        ...
        WHEN LEFT(
            ...
            OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), P.LAST_NAME)),
            5
        )
        AND LEFT(
            OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), TAAPI.orgFirstName)),
        ...
        WHEN SOUNDEX(
            ...
            OADWH_HASH.dbo.RemoveNonAlphaNumeric(P.LAST_NAME)
        )
        AND SOUNDEX(
            OADWH_HASH.dbo.RemoveNonAlphaNumeric(orgFirstName)
        ) = SOUNDEX(
        ...
        AND OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), TAAPI.orgFirstName)) = OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), P.FIRST_NAME)) THEN 76
        WHEN OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), TAAPI.orgLastName)) = OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), P.LAST_NAME)) THEN 75
        WHEN OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), TAAPI.orgFirstName)) = OADWH_HASH.dbo.RemoveNonAlphaNumeric(CONVERT(VARCHAR(25), P.FIRST_NAME)) THEN 60
        ELSE 50
    END Confidence INTO #tmpReview
FROM
    #tmpMatch TM
